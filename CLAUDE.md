...
```bash
pnpm intake <zip · tar.gz · 폴더> [--as <이름>]
```

`inbox/<이름>/` 에 풀고 목차(`INVENTORY.md`)를 만듭니다. 스택, 먼저 읽을 문서, 이미지와
해상도, 열 수 없는 디자인 파일, 키가 섞여 있으면 경고까지 정리됩니다.

**목차부터 읽으세요.** 폴더를 통째로 훑으면 컨텍스트만 태웁니다.

지키는 것:

- `inbox/` 안의 파일을 **고치지 않습니다.** 재료이지 작업 대상이 아닙니다.
- `inbox/` 안의 것을 **실행하지 않습니다.** `npm install` 도, 빌드도, 스크립트 실행도.
  남이 준 zip 은 읽을거리입니다.
- `inbox/` 는 **버전 관리하지 않습니다.** 여기서 뽑아낸 것만 저장소에 남습니다
  (코어 하드 룰 3). 근거를 인용할 때 경로만 적지 말고 **읽어낸 내용을 옮겨 적으세요** —
  나중에 `inbox/` 가 없는 사람에게는 경로가 빈 참조입니다.
