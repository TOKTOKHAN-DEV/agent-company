...
| 에이전트 → `inbox/` | 읽기만 한다. 고치지도, 실행하지도 않는다. |
...
```
zip · tar.gz · 폴더 ──▶ inbox/<이름>/ ──▶ INVENTORY.md ──▶ 에이전트
                        (버전 관리 안 함)     (목차)          (읽기만)
```

`scripts/intake.sh` 가 풀고 정리하고, `scripts/intake-inventory.mjs` 가 목차를 만듭니다.
코어입니다 — 남의 워크스페이스에서 뭔가를 받는 일은 도메인과 무관하게 생깁니다.

**왜 목차를 따로 만드는가.** 폴더를 통째로 던지면 에이전트가 파일을 하나씩 열어보며
컨텍스트를 태웁니다. 무엇이 어디 있는지 먼저 알려주면 필요한 것만 엽니다. 컨텍스트
레이어가 wiki 전문 대신 인덱스만 올리는 것과 같은 이유입니다 (ADR-0003).

**왜 버전 관리하지 않는가.** 하드 룰 3번은 "진실은 저장소 파일" 이지, "받은 것은 다
저장소에 넣는다" 가 아닙니다. `inbox/` 는 재료입니다 — 여기서 뽑아낸 명세와 에셋만 남습니다.
남의 코드와 키가 통째로 커밋되는 것을 막는 효과도 있습니다.

그래서 에이전트는 근거를 인용할 때 **경로만 적지 않고 읽어낸 내용을 옮겨 적습니다.**
`inbox/` 가 없는 사람에게 경로는 빈 참조입니다.

받은 것은 실행하지 않습니다. 심볼릭 링크는 지우고(저장소 밖을 가리킬 수 있습니다),
`node_modules` 같은 것은 걷어내고, 키로 보이는 파일은 지우지 않고 **목차에 경고로 올립니다** —
원본을 건드리지 않는 것이 원칙이고, 지울지는 사람이 판단할 일입니다.
